...
if ($no_errors == true) {
  ...
  # Verify admin role membership - create role if it doesn't exist
    if (isset($_POST['setup_admin_user'])) {
  ...
  # Verify maintainer role membership - create role if it doesn't exist
    if (isset($_POST['setup_maintainer_user'])) {
    ...
    if ($no_errors == true) {
      ...
      # Show what was created in this session
        if ($SETUP_DEBUG == true) {
            ...
            if (isset($_POST['setup_organizations_ou'])) {
                error_log("$log_prefix SETUP_DEBUG: - Organizations OU", 0);
            }
            if (isset($_POST['setup_people_ou'])) {
                error_log("$log_prefix SETUP_DEBUG: - People OU", 0);
            }
            if (isset($_POST['setup_global_roles_ou'])) {
                error_log("$log_prefix SETUP_DEBUG: - Roles OU", 0);
            }
            if (isset($_POST['setup_admin_user'])) {
                error_log("$log_prefix SETUP_DEBUG: - Admin user", 0);
            }
            if (isset($_POST['setup_maintainer_user'])) {
                error_log("$log_prefix SETUP_DEBUG: - Maintainer user", 0);
            }
            if (isset($_POST['setup_example_org'])) {
                error_log("$log_prefix SETUP_DEBUG: - Example organization", 0);
            }
        }

        print "$li_good <strong>Session Summary:</strong></li>\n";
        if (isset($_POST['setup_organizations_ou'])) {
            print "$li_good ✓ Created organizations OU</li>\n";
        }
        if (isset($_POST['setup_people_ou'])) {
            print "$li_good ✓ Created people OU</li>\n";
        }
        if (isset($_POST['setup_global_roles_ou'])) {
            print "$li_good ✓ Created roles OU</li>\n";
        }
        if (isset($_POST['setup_admin_user'])) {
            print "$li_good ✓ Created admin user</li>\n";
        }
        if (isset($_POST['setup_maintainer_user'])) {
            print "$li_good ✓ Created maintainer user</li>\n";
        }
        if (isset($_POST['setup_example_org'])) {
            print "$li_good ✓ Created example organization</li>\n";
        }
    }
}
...
if ($no_errors == true) {
    ...
    <div class="p-3 bg-light rounded">
      <form action="<?php print $THIS_MODULE_PATH; ?>">
        <input type='submit' class="btn btn-success d-block mx-auto" value='Finished'>
      </form>
    </div>
        <?php
    } else {
        ?>
    <div class="p-3 bg-light rounded">
      <input type='submit' class="btn btn-warning d-block mx-auto" value='Create new account >'>
    </form>
    </div>
        <?php
    }
    ...
    <div class="p-3 bg-light rounded">
      <form action="<?php print $THIS_MODULE_PATH; ?>/run_checks.php">
        <input type='submit' class="btn btn-danger d-block mx-auto" value='< Re-run setup'>
      </form>
    </div>
    <?php
}
