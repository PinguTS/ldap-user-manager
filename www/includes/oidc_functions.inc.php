...
function generate_oidc_auth_url($state = null) {
    ...
    if (!$OIDC_CONFIG['enabled'] || !$OIDC_CONFIG['auth_endpoint']) {
        return false;
    }
...
function exchange_code_for_tokens($code) {
    ...
    if (!$OIDC_CONFIG['enabled'] || !$OIDC_CONFIG['token_endpoint']) {
        return false;
    }
...
function get_oidc_user_info($access_token) {
    ...
    if (!$OIDC_CONFIG['enabled'] || !$OIDC_CONFIG['userinfo_endpoint']) {
        return false;
    }
...
function require_oidc_auth() {
    ...
    if (isset($_SESSION['user_id']) || isset($_COOKIE['passkey'])) {
        return false;
    }
